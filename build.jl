...
for platform in platforms
    ...
    product_hash = create_artifact() do artifact_dir
        ...
        if platform isa MacOS || (arch(platform)==:x86_64 && (platform isa Windows || (platform isa Linux && libc(platform)==:glibc)))
            ...
            run(Cmd(`$nodejs_cmd node-pre-gyp install -C $canvas_path --target_arch=$l_arch --target_platform=$l_target --target_libc=$l_libc`, dir=joinpath(artifact_dir, "node_modules", "@mapbox", "node-pre-gyp", "bin")))
    ...
    bind_artifact!(artifact_toml, "vegalite_app", product_hash, platform=platform, force=true, download_info=Tuple[("https://github.com/rben01/VegaLiteBuilder/releases/download/v$(URIParser.escape(string(version)))/$archive_filename", download_hash)])
end
